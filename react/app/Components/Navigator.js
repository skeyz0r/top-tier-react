  ...
  return (
    <nav className="flex h-[56.8px] fixed w-full z-30 justify-evenly sm:justify-between items-center border-solid border-b bg-navYellow border-gray-600">
            <Link className={`${poppins.className} sm:pl-7 text-green-600 md:text-2xl`} href={'/'}>Top Tier Services LLC</Link>
                    ...
                    <p onClick={() => { path !== '/' ? router.push('/') : srv.scrollIntoView({ behavior: "smooth", block: "start" })}} className={`${comf.className} ${vis} cursor-pointer text-green-600 md:text-xl py-3`}>Services</p>
                    <Link className={`${comf.className} ${vis} text-green-600 md:text-xl py-3`} href={'/contact'}>Contact</Link>
